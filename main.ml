...
let x_ty = ITyFun (ITyQbit, ITyQbit)
let z_ty = ITyFun (ITyQbit, ITyQbit)

let initial_env =
  List.fold [("new", New); ("meas", Meas); ("H", H); ("cnot", CNOT); ("X", X); ("Z", Z)] ~init:Environment.empty
    ~f:(fun env (id, c) -> Environment.extend env id (VConst c))
let initial_tyenv =
  List.fold [("new", new_ty); ("meas", meas_ty); ("H", h_ty); ("cnot", cnot_ty); ("X", x_ty); ("Z", z_ty)] ~init:Environment.empty
